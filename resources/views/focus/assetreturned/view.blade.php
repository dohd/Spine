                         ...
                         @isset ($assetreturneds)
                            ...
                            @foreach ($assetreturneds->item as $item)
                                ...
                                <tr class="tr">
                                    ...
                                    <input type="hidden" name="" value="{{ $asset = \App\Models\assetreturned\AssetreturnedItems::where('item_id',$item->item_id)->where('asset_returned_id', $record->id)->get()->first() }}" id="">
                                   ...
                                   @isset ($assetreturneds)
                                      ...
                                      @foreach ($assetreturneds->item as $item)
                                          ...
                                          <tr class="row1">
                                              ...
                                              <td class="text-center returned_item"><input type="text" class="form-control" name="return" value="{{ $item->returned_item }}" id="return" disabled></td>
                                              @if ($item->qty_issued > 0) 
                                                <input type="hidden" name="" value="{{ $asset = \App\Models\assetreturned\AssetreturnedItems::where('item_id',$item->item_id)->first() }}" id="">
                                                <input type="hidden" name="" value="{{ $assetReturns = \App\Models\assetreturned\AssetreturnedItems::where('item_id',$item->item_id)->where('qty_issued', '=', '0')->get() }}" id="">
                                                
                
                        
                                                @if ($assetReturns)
                                                @foreach($assetReturns as $assetReturn)
                                                <input type="hidden" name="" id="qtyIssued" value="{{$ans = $asset->lost_items - $assetReturns->sum('returned_item')}}">
                                                @endforeach
                                                
                                                        
                                                    
                                                <td class="text-center lost"><input type="text" class="form-control" name="lost" value="{{ $ans }}" id="lost" disabled></td>
                                                @endif
                                                @endif
                                              
                                              <td class="text-center broken"><input type="text" class="form-control" name="broken" value="{{ $item->broken }}" id="broken" disabled></td>
