...
@section('content')
            ...
            <div class="content-body">
                <div class="card">
                    <div class="card-content">
                        <div class="card-body">
                            {{ Form::model($purchase, [ 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'PUT', 'id' => 'data_form','files'=>true]) }}
                                <div class="row">
 ...
 <h3 class="title">{{trans('purchaseorders.bill_from')}} <a href='#' class="btn btn-primary btn-sm round" data-toggle="modal"  data-target="#addCustomer">
        ...
        <div class='col m-1'>
           @php
                                                            if($purchase->payer_type == 'walkin'){
                                                            $taxid=$purchase->taxid;
                                                            $payer=$purchase->payer;
                                                             $payer_id=0;

                                                          }else if($purchase->payer_type == 'supplier'){
                                                            $taxid=$purchase->supplier->taxid;
                                                            $payer=$purchase->supplier->name;
                                                             $payer_id=$purchase->supplier->id;
                ...
                <div class="d-inline-block custom-control custom-checkbox mr-1">
                    <input type="radio" class="custom-control-input bg-primary" name="payer_type" id="colorCheck1"
                           value="walkin"  {{ walkin=== @$purchase->payer_type ? " checked" : "" }} >
                ...
                <div class="d-inline-block custom-control custom-checkbox mr-1">
                    <input type="radio" class="custom-control-input bg-purple" name="payer_type" value="supplier" 
                           id="colorCheck3" {{ supplier=== @$purchase->payer_type ? " checked" : "" }}>
                                               ...
                                               <div class="form-group row">
                                                <div class="col-sm-8"><label for="payer"
                                                    ...
                                                    <div class="input-group">
                                                        ...
                                                        {{ Form::text('payer',  $payer, ['class' => 'form-control round required', 'placeholder' => 'Supplier Name','id'=>'payer-name',$readonly_opp]) }}
                                                    </div>
                                                </div>
                                                <div class="col-sm-4"><label for="taxid"
                                                    ...
                                                    <div class="input-group">
                                                        ...
                                                        {{ Form::text('taxid',  $taxid, ['class' => 'form-control round', 'placeholder' => 'Tax Id','id'=>'taxid' ,$readonly_opp]) }}
                                                    </div>
                                                </div>

                                                  {{ Form::hidden('payer_id',  $payer_id,['id'=>'payer_id']) }}
                                            </div>
                                                ...
                                                <div class="form-group row">
                                                 ...
                                                 <div class="col-sm-12">
                                                    ...
                                                    <div class="form-group">
                                                        <label for="project"
                                                               class="caption">Ledger Account(Credited)*</label>
                                                        <select  name="credit_account_id" class="form-control round required"
                                                                id="credit_account_id">
                                                             <option value="">Select Ledger Account</option>
                                                @foreach($accounts as $account)
                                    <option value="{{$account->id}}" {{ $account->id === @$purchase->account_id ? " selected" : "" }}> {{$account->holder}}</option>
                                                @endforeach
                                ...
                                <tr class="item_header bg-gradient-directional-blue white">
                                      <th width="20%" class="text-center">Item</th>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center" id="exp_linetotal" >{{numberFormat($purchase->sum_expense->sum('debit'))}}</td>
                                            <td class="text-center" id="item_linetotal" >0.00</td>
                                            <td class="text-center" id="totalLinetotal" >0.00</td>
                                            <input type="hidden" value="{{numberFormat($purchase->sum_expense->sum('debit'))}}" id="totalLinetotals" name="totalcredit">
                                           ...
                                           <tr>
                                            ...
                                            <td class="text-center" id="exp_disctotal">{{numberFormat($purchase->sum_expense->sum('discount'))}}</td>
                                            <td class="text-center" id="item_disctotal" >0.00</td>
                                             <td class="text-center" id="grandDiscount">0.00</td>

                                             <input type="hidden" value="{{numberFormat($purchase->sum_expense->sum('discount'))}}" id="grandDiscounts" name="granddiscounts">

                                        </tr>
                                        <tr>
                                            <td class="text-center">Tax</td>
                                            <td class="text-center" id="taxtotal">0.00</td>
                                            <td class="text-center" id="exp_taxtotal">{{numberFormat($purchase->sum_expense->sum('tax_amount'))}}</td>
                                            <td class="text-center" id="item_taxtotal">0.00</td>
                                             <td class="text-center" id="grandTax">0.00</td>
                                              <input type="hidden" value="{{numberFormat($purchase->sum_expense->sum('tax_amount'))}}" id="grandTaxs" name="grandtaxs">
                                         </tr>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center"><strong><span  id="exp_invoicetotal">
                                              {{numberFormat($purchase->sum_expense->sum('total_amount'))}}</span></strong></td>
                                  ...
                                  class="btn btn-success sub-btn btn-lg"
                                    value="Post Transaction" id="submit-data"
                                        ...
                                        <div class="inner-cmp-pnl">
                                            ...
                                            <div class="form-group row">
                                                <div class="col-sm-6"><label for="tid"
                                                    ...
                                                    <div class="input-group">
                                                        ...
                                                        {{ Form::number('tid', @$purchase->tid, ['class' => 'form-control round', 'placeholder' => trans('purchaseorders.tid'),'readonly']) }}
                                                    </div>
                                                </div>
                                            ...
                                            <div class="form-group row">
                                                <div class="col-sm-6"><label for="ref_type"
                                                                             class="caption">Reference Document Type*</label>
                                                  ...
                                                  <option value="Invoice" {{ Invoice === @$purchase->ref_type ? " selected" : "" }} >Invoice</option>
                                                  <option value="Receipt" {{ Receipt === @$purchase->ref_type ? " selected" : "" }}  >Receipt</option>
                                                     <option value="DNote"  {{ DNote === @$purchase->ref_type ? " selected" : "" }}>DNote</option>
                                                     <option value="Voucher" {{ Voucher === @$purchase->ref_type ? " selected" : "" }} >Voucher</option>
                                                ...
                                                <div class="col-sm-6"><label for="refer_no"
                                                    ...
                                                    <div class="input-group">
                                                        ...
                                                        {{ Form::text('refer_no', $purchase->refer_no, ['class' => 'form-control round required', 'placeholder' => trans('general.reference')]) }}
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                            ...
                                            class="caption">Purchase Date*</label>
                                                    ...
                                                    <div class="input-group">
                                                       ...
                                                       aria-hidden="true"></span>
                                                        ...
                                                        {{ Form::text('transaction_date', dateFormat($purchase['transaction_date']), ['class' => 'form-control round required', 'placeholder' => trans('purchaseorders.invoicedate'),'autocomplete'=>'false']) }}
                                                    </div>
                                                </div>
                                                <div class="col-sm-6"><label for="due_date"
                                                    class="caption">Due Date*</label>
                                                    ...
                                                    <div class="input-group">
                                                       ...
                                                       aria-hidden="true"></span>
                                                        ...
                                                        {{ Form::text('due_date', dateFormat($purchase['due_date']), ['class' => 'form-control round required', 'placeholder' => trans('invoices.invoicedate'),'autocomplete'=>'false']) }}
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        > {{$project->customer_project->company}} {{$project->branch->name}}-{{$project->name}}-{{$project->project_number}}</option>
                                            ...
                                            <div class="form-group row">
                                                <div class="col-sm-12">
                                                    ...
                                                    {{ Form::textarea('note', $purchase['note'], ['class' => 'form-control round required', 'placeholder' => trans('general.note'),'rows'=>'2']) }}
                                                </div>
                                            </div>
             ...
             role="tabpanel">
              ...
              <div id="saman-row-exp">
                                    <table class="table-responsive tfr my_stripe">
                                        ...
                                        <tbody>
                                           @forelse($purchase_items_tab_two as $purchase_item_tab_two)
                                    <tr>
                                            <td><input type="text" class="form-control" name="ledger_name[]"
                                                       placeholder="Enter Ledger"
                                                       id='ledgername-{{$loop->index}}' value="{{$purchase_item_tab_two->ledger->holder}}">
                                            </td>
                                            <td><input type="text" class="form-control req exp_amnt" name="exp_product_qty[]"
                                                       id="exp_amount-{{$loop->index}}" 
                                                       onkeypress="return isNumber(event)"
                                                       onkeyup="expRowTotal('{{$loop->index}}'), expBillUpyog()"
                                                       autocomplete="off" value="{{numberFormat($purchase_item_tab_two->qty)}}">
                                           </td>
                                            <td><input type="text" class="form-control req exp_prc" name="exp_product_price[]"
                                                       id="exp_price-{{$loop->index}}" 
                                                       onkeypress="return isNumber(event)"
                                                       onkeyup="expRowTotal('{{$loop->index}}'), expBillUpyog()"
                                                       autocomplete="off" value="{{numberFormat($purchase_item_tab_two->rate)}}"></td>
                                            <td><input type="text" class="form-control exp_vat " name="exp_product_tax[]"
                                                       id="exp_vat-{{$loop->index}}"
                                                       onkeypress="return isNumber(event)"
                                                       onkeyup="expRowTotal('{{$loop->index}}'), expBillUpyog()"
                                                       autocomplete="off" value="{{numberFormat($purchase_item_tab_two->tax)}}"></td>
                                            <td class="text-center" id="exp_texttaxa-{{$loop->index}}">{{numberFormat($purchase_item_tab_two->tax*$purchase_item_tab_two->qty*$purchase_item_tab_two->rate*0.01)}}</td>
                                            <td><input type="text" class="form-control exp_discount"
                                                       name="exp_product_discount[]"
                                                       onkeypress="return isNumber(event)" id="exp_discount-{{$loop->index}}"
                                                       onkeyup="expRowTotal('{{$loop->index}}'), expBillUpyog()" autocomplete="off" value="{{numberFormat($purchase_item_tab_two->discount_rate)}}"></td>
                                            <td><span class="exp_currenty">{{config('currency.symbol')}}</span>
                                                <strong><span class='exp_ttlText' id="exp_result-{{$loop->index}}">{{numberFormat($purchase_item_tab_two->total_amount)}}</span></strong></td>
                                            <td class="text-center">

                                            </td>
                                            <input type="hidden" name="exp_total_tax[]" id="exp_taxa-{{$loop->index}}" value="{{numberFormat($purchase_item_tab_two->tax*$purchase_item_tab_two->qty*$purchase_item_tab_two->rate*0.01)}}">
                                            <input type="hidden" name="exp_total_discount[]" id="exp_disca-{{$loop->index}}" value="{{numberFormat($purchase_item_tab_two->discount)}}">
            <input type="hidden" class="exp_ttInput" name="exp_product_subtotal[]" id="exp_total-{{$loop->index}}"
        value="{{numberFormat($purchase_item_tab_two->total_amount)}}">
             <input type="hidden" class="exp_pdIn" name="ledger_id[]" id="exp_pid-{{$loop->index}}" value="{{numberFormat($purchase_item_tab_two->secondary_account_id)}}">
             <input type="hidden" name="exp_taxedvalue[]" id="exp_taxedvalue-{{$loop->index}}" value="{{numberFormat($purchase_item_tab_two->taxable_amount)}}">
             <input type="hidden" name="exp_salevalue[]" id="exp_salevalue-{{$loop->index}}" value="{{numberFormat($purchase_item_tab_two->debit)}}">
                                          
                                        
                                        </tr>
                                        <tr>
                                            <td colspan="3"><textarea id="exp_dpid-{{$loop->index}}" class="form-control html_editor"
                                         ...
                                         <td colspan="4"><input type="text" class="form-control" name="exp_project[]"
                                                       ...
                                                       id='exp_project-{{$loop->index}}' value="{{$purchase_item_tab_two->project->customer->company}} {{$purchase_item_tab_two->branch->name}}-{{$purchase_item_tab_two->project->name}}-{{$purchase_item_tab_two->project->project_number}}">
                                                          ...
                                                          <input type="hidden" name="exp_branch_id[]" id="exp_branch_id-{{$loop->index}}" value="{{$purchase_item_tab_two->branch_id}}"  >
                                            </td>
                                            <td class="text-center">
                           ...
                           <input type="hidden" name="item_client_id[]" id="item_client_id-0" >
                            <input type="hidden" name="item_branch_id[]" id="item_branch_id-0" >
                                ...
                                <input type="hidden" value="new_i" id="inv_page">
                                 <input type="hidden" value="{{route('biller.purchases.update',$purchase['id'])}}"
                                   id="action-url">
