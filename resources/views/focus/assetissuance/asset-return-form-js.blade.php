{{-- <script>
    ...
    const assetissuance = @json($assetissuance);
    $('.datepicker').datepicker({ format: "{{ config('core.user_date_format') }}" })
    if (assetissuance.date) $('#date').datepicker('setDate', new Date(assetissuance.date));
    ...
    // default skill row
    let skillIndx = 0;
    let skillItems = @json($assetissuance->skill_items);
    ...
    // let productIndx = 0;
    // let assetissuanceItems = @json($assetissuance->items()->orderByRow()->get());  
    // let budgetItems = @json(@$budget_items);
    // if (budgetItems) assetissuanceItems = budgetItems;
    // assetissuanceItems.forEach(v => {
    //     let i = productIndx;
    //     if (v.a_type == 1) {
    //         // product type
    ...
    function autocompleteProp(i) {
        return {
            source: function(request, response) {
                $.ajax({
                    url: "{{ route('biller.products.assetissuance_product_search') }}",
                    method: 'POST',
    ...
    function budgetTotal() {
        ...
        // profit
        const assetissuanceTotal = accounting.unformat($('#assetissuance_total').val());
        const profit = assetissuanceTotal - total;
        let pcent = profit/assetissuanceTotal * 100;
        pcent = isFinite(pcent)? Math.round(pcent) : 0;
        $('.profit').text(accounting.formatNumber(profit) + ' : ' + pcent + '%');

        // budget limit
        $('.budget-alert').addClass('d-none');
        if (total >= (assetissuanceTotal * 0.7)) {
