...
class CreditorsController extends Controller
{
     ...
     */
    public function __construct(CreditorRepository $repository)
    {
     ...
     */
    public function index(ManageCreditorRequest $request)
    {
        $input = $request->only('rel_type', 'rel_id');
        $segment = false;
         $supplier = false;
              if (isset($input['rel_id']) and isset($input['rel_type'])) {
             $segment = Purchase::where('payer_id',$input['rel_id'])->where('payer_type','supplier')->get();
              $supplier = Supplier::find($input['rel_id']);

           // $segment = CustomerGroupEntry::where('customer_group_id', '=', $input['rel_id'])->first();

        }
        return new ViewResponse('focus.creditors.index', compact('input', 'segment', 'supplier'));
    }
