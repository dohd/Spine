 ...
 */
class SectionsTableController extends Controller
{
    /**
    ...
    public function __invoke(ManageSectionRequest $request)
    {
        ...
        $core = $this->section->getForDataTable();
        return Datatables::of($core)
            ->escapeColumns(['id'])
            ->addIndexColumn()
              
            ->addColumn('name', function ($section) {
                return '<a class="font-weight-bold" href="' . route('biller.sections.index') . '?rel_type=' . $section->id . '&rel_id=' . $section->id . '">' . $section->name . '</a>';
            })
