...
class DjcRepository extends BaseRepository
{
 ...
 if ($result) {
        ...
        foreach ($input['data_item']['tag_number'] as $key => $value) {
                 ...
                 $dataitems[] = array('djc_id' => $result->id, 'tag_number' => $input['data_item']['tag_number'][$key], 'make' => $input['data_item']['make'][$key], 'equipment_type' => $input['data_item']['equipment_type'][$key], 'joc_card' => $input['data_item']['joc_card'][$key], 'capacity' => $input['data_item']['capacity'][$key], 'location' => $input['data_item']['location'][$key], 'last_service_date' => date_for_database($input['data_item']['last_service_date'][$key]), 'next_service_date' => date_for_database($input['data_item']['next_service_date'][$key]), 'ins' => $result->ins);
